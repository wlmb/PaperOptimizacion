...
\notaL{Any reason
	for these choices? They seem arbitrary.} \notaL{Creo que alguna 
	explicación  sencilla merece el lector.} 
\notaV{Agregue:  }
Approximately $90\%$ of the solar radiation that reaches the 
earth's surface is contained in the spectral region of 350 to 1400 
nm ($\mathcal R_1$), while in ($\mathcal R_2$) it can be up 
to $98\%$ of terrestrial solar radiation \cite{Bird1983}. A direct 
...
\notaL{¿Sería
	...
	nuestra definición?}
\notaV{Agregue:   }
Various photonic structures have been obtained with $R>90\%$ 
\cite{Xu2018,Ahmed2014,Reiner2000}, so we also report 
omnidirectional spectra band (ODB) with $R>90\%$ for all angles.
...
structure has an ODB of width (ODW) 250 nm centered at (ODC) 1125 nm, as shown in Fig. \ref{Fig1}(b). 
...
\notaV{Agregue:   }
We define the percentage of overlap as the portion of the PBG of the $j$-th mirror that has been covered by the PBG of the $(j + 1)$-th mirror.
...
at 1160 nm and 1925 nm, respectively. We fabricated a
...
\notaV{Agregue:    } Thus, the results we have shown are very good for all three structures.
%Therefore, the second technique is found to be better as it results in thiner structures with a noticeable improvement in the average reflectance.
...
\notaV{Agregue:   }
Furthermore, if someone without experience wants to obtain an 
optimized photonic structure, we recommend using the chirped type 
structures. There are cases in which it is suggested to use 
sub-mirror stacking, for example, when the absorption of the 
