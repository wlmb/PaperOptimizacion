...
structures and by \textit{stacking} sub-mirrors. We find \notaL{No
  estoy seguro del tiempo} that
chirped structures are more suitable for the Vis range, while stacked
sub-mirrors are better for the NIR region. \notaV{Corregi oracion} We design, fabricate and test some
...
crystal, their propagation is \notaV{Quite "will be"} forbidden if their energy lies
...
thicknesses are \notaV{Modifique "of is"} quarter-wavelength
$n_{H}d_{H}=n_{L}d_{L}=\lambda_{0}/4$ at a \notaV{Quite "some chosen"} nominal
...
\notaV{Se modifico la oracion "An alternative is to stack BM's, such that their PBG
...
On the other hand, different fabrication techniques have been used to obtain omnidirectional
mirrors (ODM) that operate on the visible and/or near infrared (Vis-NIR) range of the
electromagnetic spectrum. \notaV{Se modifico la oracion "Till now, different deposition 
techniques have been used to obtain omnidirectional BM (ODBM) that operate on 
the visible and near infrared (Vis-NIR) range of the electromagnetic spectrum."} 
For example, Chen et al. \cite{Chen1999},
...
6-pair TiO$_{2}$/SiO$_{2}$ structure \notaV{Se cambio "have reported"} with
...
time to the electrochemical reaction, it is possible to obtain 
multilayer structures allowing for fabrication ODM \cite{Xifre2015,Pavesi2000}.
\notaV{Se replanteo la idea "as PS may be fabricated by simple
...
chirped multilayer structures \notaV{Modifique "quasi-omnidirectional Bragg mirrors"} has
...
PS based dielectric optical filters, quasi-ODBM and ODBM have been extensively studied in different
regions of the electromagnetic spectrum, such as the ultraviolet (UV)
\cite{Jimenez2020}, visible \cite{Ariza2012}, and NIR \cite{Bruyant2003}. 
...
we study different strategies to produce multilayer \notaV{photonic} mirrors
that have a large reflectance omnidirectional band \notaV{over a wide band-width}
...
\notaV{Propuesto por Dr. David, se omite "Theory"}
...
be as simple or complex as you like. The proposed function was 
optimized with the Minuit module of PDL (Perl Data Language) \cite{minuit}. 
...
$R=|r|^{2}$ and  $T=\beta \left\vert t\right\vert ^{2}$, respectively, with
\begin{equation}
...
and the substrate, respectively. $\beta =\text{Re}Z_{0}/\text{Re}Z_{s}$ is choosen for 
...
polarization. Furthermore, $M_{ij}$ are the elements
...
TM polarization. Moreover, sub-index $0$ and $s$ 
refer to the incident and substrate media, respectively.
The complex refractive index of the porous layers were 
...
\notaV{Se omite "For the analysis of the propagation of 
electromagnetic fields through multilayer systems it is 
usual to employ the transfer matrix method \notaL{reference} 
\cite{Pochi2005, Ariza2011}. Assuming the system is 1D, with 
variations only along the $z$ directions, and that the 
polarization of light is either transverse electric (TE) or 
transverse magnetic (TM)\notaL{si no, la matriz   sería de 4x4}, 
the transfer matrix $M\left(z_{2},z_{1}\right)$ is a 2x2 matrix 
that relates the components of the electric $E_{\| }$ and 
magnetic  $H_{\|}$ fields parallel to the $xy$ plane evaluated 
at $z_2$ to their value at $z_{1}$,
\begin{equation*}
\left(
\begin{array}{c}
E_{\|} \\
...
\right) _{z_{1}}.
\end{equation*}%
Many equivalent formulations have been proposed to obtain and use $M$ \cite{Mochan1987,Ortiz2020,Chavez2020}. Here we use a recently developed formalism for the numerically stable calculation of the reflectance of large multilayer systems, as proposed by Puente-Díaaz, et al. \cite{Puente2020}, summarized in the supplementary information. The explicit expressions for the optical coefficients are: 
\begin{equation}
r=\mp \frac{Z_{0}M_{11}+M_{12}-Z_{0}Z_{s}M_{21}-Z_s
M_{22}}{Z_{0}M_{11}-M_{12}-Z_{0}Z_{s}M_{21}+Z_{s}M_{22}},  \label{rBloch}
\end{equation}%
...
\end{equation}%
where $M_{ij}$ \notaL{Quité las tildes, pues hubiera sido 
necesario explicarlas, y no creo que sean importantes para 
el lector} are the elements of the transfer matrix which 
relates the electromagnetic fields of all interfaces in 
the structure through transfer the fields from its surface 
interface with vacuum at $z_0$ towards the substrate at $z_s$,
$Z_0$ is the surface impedance of vacuum and $Z_s$ is the 
surface impedance of the substrate, and we define $r=F_r/F_i$ 
and $t=F_t/F_i$ in terms of the reflected and transmitted 
fields, where we choose the $F$'s as electric fields for 
(TE) polarization and magnetic fields for transverse 
magnetic polarization (TM).  The upper sign $-$ in Eq. 
(\ref{rBloch}) and the subscript $\alpha =s$ in Eq. 
(\ref{tBloch}) are chosen for the case of TE polarization, 
while the lower sign $+$ and the subscript $\zeta =0$ 
correspond to TM polarization, respectively. The reflectance 
is given by $R=|r|^{2}$ and the transmittance by 
$T=\beta \left\vert  t\right\vert ^{2}$ with 
$\beta =\text{Re}Z_{0}/\text{Re}Z_{s}$ for TE polarization 
and $\beta =\text{Re}Z_{s}/\text{Re}Z_{0}$ for TM 
polarization.\notaL{En el cálculo, ¿tomaste las partes 
reales? No estaban en la fórmula} The complex refractive 
index of the porous layers were obtained through the 
Bruggeman's effective medium theory, which has been 
reported to adequately reproduce the PS optical 
parameters \cite{Giusseppe2011,Pap2006,Estrada2018}.}
...
\notaV{Se omite "With this electrolyte, the minimum and maximum porosities that can be obtained are 35$\%$ and 76$\%$ \notaL{Creo que se puede quitar de   aquí, pues lo discutes abajo:(gravimetrically obtained)}, using current densities of 0.5 and $305\text{mA}/\text{cm}^{2}$, respectively.} 
...
\notaV{Se reescribe "The porosity was calibrated \notaL{Creo que no 
muestras curvas de calibración: calibration curves were acquired}
through a gravimetric technique as follows: single layers of porous
silicon were synthesized under similar conditions applying a constant
current for a time \notaL{cuánto tiempo para la calibración?} and
were weighed to obtain their mass $m_i$ before ($m_{1}$) and after 
($m_{2}$) the electrochemical attack, and after dissolving the already
formed porous silicon layer ($m_{3}$), to obtain the porosity as 
$p=(m_{1}-m_{2})/(m_{1}-m_{3})$ \cite{Pap2006}.}
The porosity of the layers was obtained by a previous calibration
curve through a gravimetric technique \cite{Pap2006} 
\notaV{Si es que se adjunta (further details see supplementary
information), sino ya se ha mencionado en otros articulos}. The 
...
electromagnetic spectrum is by stacking sub-mirrors tuned to different
wavelengths \cite{DelRio2018,Agarwal2003}. Such grouping must meet the condition
imposed by the dispersion relation,\notaL{No se entiende, ¿de qué la gira
la relación de dispersión? Es la relación de dispersión ¿de qué? Falta
una explicación!} $\cos KD=\frac{1}{2}\text{Tr}\,M$, where $D$ is
the period, which corresponds to the actual thickness of the
sub-mirror\notaL{Por qué? El espejo podría tener varios periodos.}, $\pm K$
represents a 1D Bloch's vector corresponding to a wave that propagates along
the $z$-direction \cite{Mochan1987,Mochan1988,Perez2018}, and T$\text{r}$ denotes the
...
be able to propagate through the structure. \notaL{Las ideas están
  bien, pero está muy mal explicado, en desorden}

The ideal stack of sub-mirrors would be that edge where the PBG of the $j$-th mirror begins
is close to the  edge of the where the PBG of the $j-1$-th mirror
ends, and sufficient mirrors are stacked until the desired interval
bandwidth has been
covered. However, this arrangement does not give the maximum average reflectance due to
the decrease in the reflectance at each intersection of the two
PBGs.\notaL{QUé es una intersección de PBG? Escribe para que el lector
  entienda} On the other hand,
...
(Fig. \ref{Fig4}(a)). \notaL{No entendí el argumento, ni cómo
  dterminaste para qué lambdas se usan tantos o cuántos periodos. ¿Fue
  prueba y error? ¿Fue una optimización?}\notaL{Por cierto,
  ¿optimizaste qué tanto traslapar espectros sucesivos o usaste la
  regla de que se toquen los bordes de banda?}
\begin{figure}[tbph]
\begin{center}
\includegraphics[width=\textwidth]
{F6OptimizedNIRSE.pdf}
\end{center}
\caption{\notaL{Stack, no stak. ¿Qué significa la región rugosa que
    rodea la estructura?} (a) Diagram showing the distribution
  of sub-mirrors and the corresponding
number of periods in a PS multilayer.\notaL{Esto no es un cartel. No
  entiendo qué pretende ilustrar el panel (a)} (b) Maximization of the average reflectance as
a function of the overlap photonic band gap for the structures formed by sub-mirrors
with a fixed number of periods \notaL{No has explicado a qué te
  refieres con esta optimización. No has explicado qué es el overlap
  photonic band gap. Esto parece tomado de un cartel, pero el artículo
debe ser autocontenido; no estás ahí parado para explicar.}(3 and 4),
and for structures with the design of (a)
using 3 and 6 periods for the tuned sub-mirrors in $\lambda>800$
nm.\notaL{No entendí nada.} (c)
Calculated reflectance for the optimized structure for non
polarized light. (d) Comparison between the measured and calculated
reflectance spectra at several angles of incidence. The gray band indicates the region in which the
reflectance is greater than 95$\%$. Inset shows a photograph with a top view of the
...
of 1 (400, 460 nm), 2(510, 570, 640 nm), 3 (720 nm) and 6 (810 nm and
above).\notaL{Sugiero reescribir tratando de que la explicación sea clara}

The reflectance mapping shown in Fig \ref{Fig4}(c) reveals a quasi omnidirectional
mirror in the range from 950 to 2900 nm, for $\theta$ between
$0^\circ$ and $60^\circ$. It is also observed that at incidence angles $>$70${{}^\circ}$, for long
...
by Puente-D\'{\i}az, et al \cite{Puente2020}. Furthermore, in the supplementary
information we show the trace of the transfer matrix as function of the
wavelength and the incidence angle for the ST-C photonic structure for TE and
TM polarizations.\notaL{¿Para qué ponemos esta información
  suplementaria? ¿Qué aprenderá el lector de ella?} In Fig
\ref{Fig4}(d) we compare the calculated and measured reflectance
for the structure ST-C, at several angles of incidence.
Although, the quasi omnidirectional wavelength range of the synthesized structure is
slightly smaller than the calculated one, from 950 to 2750 nm, the measured PBG
is almost twice as wide as those of recently reported similar structures
(\cite{DelRio2018,Chavez2020}).

Finally, table 2 shows some porous silicon based ODBMs \notaL{Me
  preocupa un poco la nomenclatura: si no son periódicos, ¿son espejos
  de Bragg?} designed to operate in the
...
than recently reported works.\notaL{Es justa la comparación? Ellos
  estaban tratando de hacer un espejo de espectro muy amplio y no
  pudieron hacerlo tan ancho como nosotros o simplemente estaban
  haciendo otra cosa?}
...
We have designed, fabricated and characterized highly reflective PS multilayer photonic
structures with a wide spectral range, optimized using two techniques for maximal reflectance
and minimal thickness in the NIR region \notaL{Parte del trabajo no
  fue NIR.} With \textit{chirped}-type
Bragg\notaL{Tengo la duda sobre llamarles Bragg} mirrors,
two regions of the electromagnetic spectrum were analyzed. The first region (350nm -
1400 nm), optimized through an increasing function resulted in a
reflectance $R>0.85$ with a range of 360 nm centered at 1160 nm
(980-1340 nm) for the angular range $0-60^\circ$ and $R>0.50$ of
reflectance from 550-980nm. The second structure was designed for a
wavelength range from 850-3000 nm
and the synthesized structure with optimized parameters resulted in
the average reflectance of $~91\%$ with q-OD \notaL{No definiste q-OD
  nunca} bandgap from 980 to 2780 nm and a
thickness of 60.4 $\mu$m. The other design technique consists of
stacking sub-mirrors
tuned to different wavelengths, and was optimized for a maximum average
reflectance using  the overlap of the gap of succesive sub-mirrors as
the optimization parameter. We obtained
a reflectance $R>0.95$ and a thickness of 41.5 $\mu$m within a
wavelength range of 1800 nm centered at 1850 nm, with  an
angular range $0-60^\circ$. Therefore, we found this second technique to be
better as it yields thinner structures by a factor of $~1.5$ and
...
multilayers or in systems composed of other types of materials. Such
