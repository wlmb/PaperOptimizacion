...
\notaV{S\'{i}, la estructura m\'{a}s reciente tiene un ODW de 1321 nm (compuesta por LHM) y la \'{u}ltima estructura de SP tiene un ODW de 1000 nm; las nuestras tienen un ODW de 1800 nm.}
\end{abstract}
...
$\lambda_{\min}=850$ nm and $\lambda_{\min}=3000$ nm. \notaL{Any reason
  for these choices? They seem arbitrary.} \notaV{El objetivo de elegir la primer regi\'{o} era comparar la estructura optimizada con el espejo blanco obtenido por Ariza \cite{Ariza2012}. La segunda regi\'{o}n la eleg\'{i} porque es uno de los rangos m\'{a}ximos que puedo medir (200-3300) nm. En principio puedo optimizar en una regi\'{o}n m\'{a}s amplia, pero solo quedar\'{i}a en el c\'{a}lculo sin su complemento experimental.}
%The
...
\begin{table}
  ...
  \caption{Optimized parameters $\alpha$, $\beta$, $A$ and $N_p$ yielding the highest
    ...
    width and center (ODW and ODC). \notaL{Para el primer rango habría que
      especificar $\lambda _{min}$ también.} \notaV{Listo}}
\end{table}
...
\notaL{Se me ocurre ODW, omnidirectional spectral width, ODB band, ODC center. Si están de
  acuerdo, habría que rotular de nuevo las figuras.} \notaV{De acuerdo! Figuras modifocadas.}
of width (ODW) 250 nm centered at (ODC) 1125 nm, as shown in Fig. \ref{Fig1}(b). \notaL{¿Por
  qué 90 y no 80 o 95? ¿Es arbitrario? ¿Es un estandard? ¿Hay alguna referencia que defina dicho criterio?} 
\notaV{Alguna referencia que defina como tal este criterio, NO. 
	Sin embargo, hay algunos autores que toman $R>90\%$ como punto 
	de partida para describir sus sistemas multicapa 
	\cite{Xu2018,Ahmed2014,Reiner2000}. Depende de los resultados 
	que hayamos obtenido y qu\'{e} queramos vender}
...
first sub-mirror consisted of 2.5 periods \notaL{This means 5 layers?} \notaV{S\'{i}, as\'{i} lo propuso} tuned to
$\lambda_{1}=700$ nm; the following $j-th$ sub-mirrors were tuned to
wavelengths $\lambda_{j}$ according to the sequence $\lambda_{j+1}
- \lambda_{j}= \left(2+j \right)$ nm %(with its suitable units)
\notaL{This is still
  wrong and/or I don't understand. What are these suitable units? \notaV{units: nm} Why
  the mistery? \notaV{No cre\'{i} que fuera tan relevante} $j$ numbers
  periods or sub-mirrors? \notaV{$j-th$ sub-mirrors} Do you mean (2+j) nm, such as the sequence
  $\lambda_1=700$, $\lambda_2=700+2+2=704$, $\lambda_3=704+2+3=709\ldots$
  $\lambda_{54}=700+2*53+54*55/2-1=2290$?} 
\notaV{$\lambda_1=700$, $\lambda_2=\lambda_{1}+2+1=703$, $\lambda_3=\lambda_{2}+2+2=704\ldots$
$\lambda_{54}=\lambda_{53}+2+53= 700+2*53+53*54/2= 2237$.}
with 4 periods for
...
\begin{equation}
	 \cos \left(KD \right) = \frac{1}{2} \text{Tr} \bm M,   \label{Dispersion}
\end{equation}
...
4 and 10. \notaL{¿Por qué no se optimizó $p$? Se podría optimizar para cada subespejo?}
\notaV{De acuerdo a lo aprendido en las estructuras chirped, se 
	requiere que las estructuras posean pocas bicapas en la 
	regi\'{o} de alta absorci\'{o}n y pueden ir aumentando a 
	medida que disminuye la absorci\'{o}n. Por eso se me hizo 
	sensato proponer esa configuraci\'{o}n en la segunda 
	estrategia de diseño. S\'{i}, si se podr\'{i}a optimizar $p$ 
	para cada sub espejo; sin embargo, corremos el riesgo de 
	obtener una estructura similar a la propuesta por 
	Estrada-Weise \cite{DelRio2018}. Adem\'{a}s, los resultados 
	aqu\'{i} mostrados ya no ser\'{i}an los adecuados.}
...
\notaV{No entend\'{i} esta observaci\'{o}n.}
...
silicon based OD mirrors designed to operate in the NIR
region and the width of the OD bands are compared with those of 
the structures analyzed in the present study. For example, the ODW 
of the structure $S_1$ is 20 nm wider than that reported by Bruyant, 
...
synthesized by Estevez, et al. \cite{Estevez2009}; however, the width of S3 extends to 2750 
nm. In other words, $S_3$ has ODW 3.55 times higher. Moreover, the 
ODW of the structure reported by Xifre-Perez, et al. \cite{Xifre2005} represents only 17.6 
\% of the ODW of the S2 and S3 structures. Aditionally, the table 
...
largest reported work until now in the NIR region \cite{Wu2021}. \notaL{Creo que podríamos
  ...
  con mejores anchos de banda} \notaV{Agregue tres oraciones m\'{a}s}
...
at 1160nm and 1925nm, respectively.\notaL{Quité acrónimos de las conclusiones} \notaV{Perfecto}. We fabricated a
...
reflectance.\notaL{No me es clara la conclusión, pues cada muestra fue diseñada para distintas
  ...
  buenos para las tres muestras. \notaV{De Acuerdo} Si alguien quisiera diseñar un espejo específico, ¿sabrías qué
estrategia recomendarle para su diseño?}
\notaV{Si el lector no tiene experiencia acerca de los materiales, 
	se aconsejar\'{i}a usar estructuras chirped. Por otra parte, al observar el mapeo de los valores de las trazas de sub 
	espejos, se podr\'{i}a diseñar la estructura adecuada a las 
	necesidades.}
