...
maximize the reflectance ($\braket{R_{\text{ave}}}$ \notaL{El promedio no depende
  de lambda ni theta, y prom es castellano} averaged over a wide wavelength
and angular range  of PS-based mirrors with different
porosity contrasts searching for a minimum thickness. Optimization is
...
structures and \textit{stacking} sub-mirrors. The
chirped structures were better suited for the Vis, and the stacked
sub-mirrors for the NIR regions. We design, fabricate and test some
...
Photonic crystal (PhC)-based structures are extensively investigated due
...
where transmission states are forbidden. The PhC  shows a
...
one-dimensional (1D) and two-dimensional (2D) PhCs for its
...
\cite{Kumar2020}. The simplest 1DPhC is composed of alternating high $\left(
...
n_{H}d_{H}=n_{L}d_{L}=\lambda_{0}/4$, at some operating wavelength $\lambda_{0}$ 
(center wavelength) to obtain a certain
...
wavelength. Furthermore, Jena et al., \cite{Jena2019}
...
layers) to generate 1DPhC of TiO$_{2}$/SiO$_{2}$ and achieved ODBM from
592 to 668 nm. However, these techniques are expensive and
require sophisticated equipment and long fabrication time. Using PS
...
fact opens the possibility of designing 1DPhC with PS, which seeks to
...
reference). Under the consideration that the system only varies along the $z$ axis, the 
transfer matrix is $M\left(z_{2},z_{1}\right)$
...
\right) _{z_{1}}.
\end{equation*}%
Many equivalent formulations have been proposed to obtain $M$ 
...
\cite{Canham1990,Escorcia2007}. Electrochemical anodization process was
...
the porosity as $p=(m_{1}-m_{2})/(m_{1}-m_{3})$ \cite{Pap2006}. Etching
rate of PS was obtained by re-synthetizing individual layers under similar
conditions and measuring their thicknesses through SEM (Scanning Electron
Microscopy). The absolute reflectivity measurements were carried out
