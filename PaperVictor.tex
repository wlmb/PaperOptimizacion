...
average reflectance $\braket{R}$ \notaL{No entiendo por qué insisten
  ...
  símbolos de menor y mayor no corresponden a los de promediar.}
\notaV{Fue un error mio. No habia agregado el paquete "braquet"}. of PS-based mirrors is maximized within a wide
...
visible (Vis) and near infrarred (NIR) regions of the electromagnetic spectrum. We design the
mirrors  \notaL{No entiendo para qué haber cambiado el
  modo de activo a pasivo; es mucho mejor el activo.} 
\notaV{Cambie la oracion a voz activa} using \textit{chirped}-type structures and \textit{stacking}
...
\notaV{Regreso el parrafo de la primera versi\'{o}n}
...
\notaL{Creo que no se ha explicado el proceso de diseño. No sé para
  qué damos tanto detalle sobre cómo se calculó, dado que todo está en la referencia.}
\notaV{El proceso de diseño se explica en la secci\'{o}n de discusi\'{o}n y \}resultados}
...
\notaV{En el siguiente parrafo se explican los procesos de diseño}
...
\notaL{I'm unsure about the nomenclature ``gap''. I guess it is
  theoretical, right?}
 \notaV{Podr\'{i}a utilizar el t\'{e}rmino "OD width" para referirme al ancho omnidireccional del espejo}
...
structure. 
\notaV{replanteo la idea: "For $\mathcal R_2$ it is only necessary that the periods have an adequate ordering.}
\notaL{No entendí qué hay que comentar en
  este caso} 
Due in $\mathcal R_2$, the absorption of porous silicon is negligible, the periods are distributed quasi-evenly.
...
$\theta > 40^\circ$ both spectra are qualitatively consistent} \notaV{De acuerdo. Se puede omitir}
The
...
different criteria. 
\notaV{I rewrote this "For example, Agarwal et al. 
	\cite{Agarwal2003} proposed to stack sub-mirrors as follows: 
	tune the first sub-mirror to certain wavelength $\lambda_{1}$. 
	Tune subsequent submirrors $j$ to wavelengths $\lambda_j$, 
	chosen so that $\lambda_{j+1} - \lambda_{j}=2+j$."} 
For example, Agarwal et al. \cite{Agarwal2003} proposed a photonic 
structure in which she stacked 54 sub-mirrors as follows: the 
first sub-mirror consisted of 2.5 periods and tuned it to 
$\lambda_{1}=700$ nm; the following sub-mirrors were tuned to the 
wavelengths $\lambda_{j}$ according to the sequence $\lambda_{j+1} 
- \lambda_{j}=2+j$ (with its suitable units) with 4 periods for 
...
and $M$ is the matrix given by Eq. \ref{MsBloch}}. \notaV{Entendido, gracias} Thus,
...
$\lambda_{D} \geq 800$ nm, the number of periods was variable.
\notaL{No se entiende. Los números uno, dos, tres ¿se sacaron de la manga? ¿se optimizaron? Para <800, ¿de dónde se sacó el número de periodos?}
\notaV{Los números de periodos uno, dos, tres, los eleg\'{i} para 
	que la estructura tenga pocos periodos en la region visible. 
	Adem\'{a}s, del c\'{a}lculo de las Trazas y reflectancias de 
	los subespejos individuales sintonizados de 1000 a 2500 nm, 
	tambi\'{e}n tienen una regi\'{o}n de propagaci\'{o}n prohibida 
	en el intervalo de 400 a 900 y la reflectancia es mayor al 
	80\%. Esto se puede observar en la Fig. SITrazasyRefSE.pdf} 
\notaV{Tambi\'{e}n fue a prueba y error}
In both designs, the optimal overlap of two consecutive PBGs was 
obtained that maximizes the average reflectance $\braket{R}$.
\notaL{No quisiste decir average reflectance $\braket{R}$?. Si sí, en qué rango promediaste?} \notaV{Si, se promedi\'{o} en el siguiente rango}
Reflectance spectra were calculated for
$400\text{nm}\le \lambda\le 3000\text{nm}$ and for $0^\circ\le\theta\le90^\circ$.

Table \ref{tab:table2} shows the number of periods of each sub-mirror ($p$),
\notaL{Está difícil esta notación. ¿De dónde vienen la s y
  la e. $Np_{se}$ parece el producto de $N$ con $p_{se}$. ¿De dónde la p?} \notaV{Lo cambi\'{o} por $p$ de periods y $N$ de N\'{u}mero total de periodos}
the total number of stacked sub mirrors ($N$)
\notaL{El encabezado de la tabla está mal} \notaV{Corregido}, the
...
to the design wavelength. \notaL{No entiendo nada: qué dejaste 
	fijo y qué permitiste variar para optimizar en cada bloque de la tabla?}
\notaV{En el primer bloque \'{u}nicamente se optimiz\'{o} el traslape de dos PBGs consecutivos. En el segundo bloque, se optimizaron el n\'{u}mero de periodos (de 4 a 10 periodos) de los subespejos sintonizados a $\lambda_{D} \geq 800$ nm y el traslape de dos PBGs consecutivas.} 
\begin{table}[htbp]
  \begin{center}
    ...
    \begin{tabular}{|c|c|c|c|c|}
      \hline % <-- Toprule here
      \textbf{$p$}&{\textbf{$N$}} &\textbf{Thickness}&\textbf{Overlap}&$\braket{R}$\\
			&&($\mu$m)& \textbf{PBG $\%$}&$\%$\\
      \hline
      1&98&43.2&95&91.0\\
      3&31&42.5&81&91.8\\
...
the synthesized structure is slightly smaller than the calculated 
one, from 950 to 2750 nm, the measured PBG is almost twice as wide 
as a recently reported PS chirped structure \cite{Chavez2020}, his 
structure has the largest reported OD width.. 
\notaL{Por qué comparamos con ésta? 
	¿Era la mejor que había? Habría que decirlo}
\notaV{S\'{i}, es la estructura, a base de SP, que tiene el mayor ancho OD} 
